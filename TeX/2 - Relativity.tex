...
Paper~3~\cite{otto-gravity}.\ This inflow is the direct cause of gravitational time
...
Paper~3~\cite{otto-gravity} for the detailed conservation analysis).
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Gravitational lensing.}\ Light propagating through the inflowing
    ...
    matching general relativity (Paper~3~\cite{otto-gravity}).
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Relativistic $\mathbf{E}$/$\mathbf{B}$ field mixing.} In special relativity, a pure electric field in one
    ...
    stationary charge has a spherically symmetric coherent spin-phase field
    ($\mathbf{E}$ only), but in a frame where the charge moves, the emitted
    wavefronts are Doppler-tilted by the motion, developing angular structure
    ($\mathbf{B}$ appears).
