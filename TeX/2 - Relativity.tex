...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Relativistic $\mathbf{E}$/$\mathbf{B}$ field mixing.} In special relativity, a pure electric field in one
    ...
    stationary charge has a spherically symmetric coherent axial-oscillation wave field
    ($\mathbf{E}$ only), but in a frame where the charge moves, the wavefronts are
    Doppler-tilted by the motion, developing angular structure ($\mathbf{B}$ appears).
