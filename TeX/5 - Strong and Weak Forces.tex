...
arise from \emph{unbound} Aether: organized flow patterns, coherent spin-phase
pumping by bound core rings, and the residual asymmetry of transient binding.\
But the theory's central mechanism---the ability of Aether to \emph{bind} into
matter---has not yet been applied to the strongest force in nature.
...
When Aether binds permanently (as described in Paper~1~\cite{otto-overview}), the bound material no longer
...
because the sourced spin-phase flux spreads over a sphere (Gauss's law).\ The
strong force does not weaken because the bridge does not dilute---it is a one-dimensional
material strand, not a three-dimensional wave field.
...
(Paper~7~\cite{otto-quantum}).\ This means the bridge has no extended
...
superfluid throughout this process (Paper~7~\cite{otto-quantum})---it is
...
Paper~7~\cite{otto-quantum}), because the deconfinement condition probes
...
used in Paper~7~\cite{otto-quantum} for the homogeneous condensate, applied here
...
(Paper~7~\cite{otto-quantum}).\ The spin stiffness $K_{\mathrm{spin}}$
...
$m_A \approx 616$--$644$~MeV/$c^2$.\ The compression ratio $X$ along the
Coulomb curve is $\approx 1.0\times10^{8}$ ($\log_{10}X \approx 8.0$), about
$50\%$ above the electron-mass-only value $\approx 7\times10^{7}$ quoted in
this paper, with bridge sound speed
$c_s = c/\sqrt{X} \approx 2.9\times10^{4}$~m/s; the difference reflects the
joint-consistency requirement on the core-energy fraction
($f_{\mathrm{core}} \approx 0.18$--$0.26$) discussed in Paper~7(a).}
