...
\subsection{Topological vs.\ Geometric Charge}

The resolution depends on the nature of the electron's charge in the
Aether model.\ Paper~4 \cite{otto-em} defines charge as ``the direction
of the Aether flow vector''---a ``radial inflow of Aether toward the
particle'' that is ``distinct from the circulatory vortex that produces the
magnetic moment'' and is ``linear, non-rotational, and spherically
symmetric around the charge.''

There are two possibilities:

\begin{enumerate}[leftmargin=2em]
  \item \textbf{Geometric charge} (distributed on the ring).\ The radial
    inflow originates from the vortex tube distributed around the $386$~fm
    circumference.\ At distances $\gg \rloop$, the flow averages to
    spherically symmetric, but at $r \lesssim \rloop$ the ring geometry is
    visible.\ This predicts $F(Q^2) \neq 1$ at $Q \sim 0.5$~MeV---ruled
    out by experiment.

  \item \textbf{Topological charge} (from the winding number).\ The radial
    inflow is sourced by the vortex \emph{topology}---the winding number
    itself, not the spatial geometry of the ring.\ The charge is a
    property of the topological defect, which has no spatial extent.\ This
    predicts $F(Q^2) = 1$ at all~$Q$, consistent with experiment.
\end{enumerate}

The distinction is analogous to a magnetic monopole: a monopole's charge
is topological (from the field configuration's winding number), not
geometric (from a spatial charge distribution).\ Its form factor is
$F(Q^2) = 1$ because there is no charge distribution to resolve---the
charge \emph{is} the topology.

Paper~4's description of charge as ``spherically symmetric'' radial inflow
is consistent with option~(2): the spherical symmetry is exact (not
approximate), because it arises from topology rather than from geometric
averaging of a ring source.\ The vortex ring carries mass (circulation KE)
and spin (angular momentum) distributed over the $386$~fm circumference,
but the charge is a topological property of the vortex core with no
spatial extent.

\subsection{Status}

Whether the electron's charge is topological or geometric is the
acknowledged open question from Paper~4 \cite{otto-em}: the microscopic
mechanism connecting the vortex topology to the radial inflow has not been
derived.\ If the charge is topological, the form factor challenge is
resolved.\ If it is geometric, the vortex model is ruled out by existing
scattering data.\ A first-principles derivation of the photon--vortex
coupling in the condensate would settle this question.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Form factor.}\ See Section~\ref{sec:form-factor} for a
    detailed analysis.\ The key open question is whether the electron's
    charge is topological (from the winding number) or geometric
    (distributed on the ring).\ If topological, $F(Q^2) = 1$ at all $Q$,
    consistent with experiment.

  \item \textbf{Charge origin.}\ The bound core ring surface is a permanent
    DCE boundary with intense transient cycling.\ Could this be the
    microscopic mechanism that implements the radial Aether flow defining
    electric charge---an acknowledged gap in
