...
from first-sound perturbations (Paper~7~\cite{otto-quantum}), so the
effective photon speed at macroscopic wavelengths is the two-fluid
first-sound speed $c_1$, not the bare condensate speed~$c$.\ (The
\emph{static} Coulomb interaction, by contrast, is carried by the spin
sector's kinetic phase stiffness alone and is insensitive to these
propagation-speed distinctions; Paper~7(a)~\cite{otto-boundcore}.)\ The viability of superfluid dark
...
acoustic metric of Paper~3~\cite{otto-gravity} to position-dependent
...
universality is given in Paper~3~\cite{otto-gravity}.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Charge:} determines the net spin-phase flux sourced
    by the bound-core structure of the infalling matter (electric field;
    Paper~4~\cite{otto-em}).\ A net positive or negative charge produces a
    persistent long-range spin-current field, its sign set by the net
    winding-pump direction, superimposed on the gravitational inflow.
\end{itemize}
...
regime of the strong-force bridges described in Paper~7~\cite{otto-quantum}.\
...
Paper~5~\cite{otto-strong}.

The bridge clarification developed in
Paper~7~\cite{otto-quantum}---that bridges are elastic compressions
...
\begin{itemize}[leftmargin=2em]
  ...
  \item The no-hair theorem follows from the sonic horizon: only mass (inflow profile),
    angular momentum (rotational flow), and charge (sourced spin-phase
    flux) produce long-range Aether patterns that survive outside the horizon.\ All
    other information is swept inward.
