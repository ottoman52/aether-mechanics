...
microscopically a spin-wave excitation; in the bound-state picture of
Paper~4, this flow vector is the effective velocity field of the coherent
spin-phase current sourced by a charged particle's bound core ring (its
winding pump; Paper~7(a)~\cite{otto-boundcore})---a localized, sourced form
of the same fundamental spin-wave degree of freedom that propagates as free
...
detected?\ Every atom generates a near-field coherent spin-phase wave field
around its nucleus (its electric field;
Paper~4~\cite{otto-em}), whose time-averaged effective pressure envelope
creates a velocity gradient that is strongest near the nucleus.\ When a propagating wave
enters this strong-gradient environment:
%
\begin{enumerate}[leftmargin=2em]
  \item The pressure-gradient refracts the wave inward, compressing it spatially---analogous
...
deposit at a specific atom.\ This is not a ``wavefunction collapse'' requiring an
external postulate---it is the natural behavior of a spin-wave excitation entering a
strong-gradient region bounded by oscillating vortex structures.\ (The Derrick analysis
of Section~\ref{sec:derrick} applies to the \emph{uniform} condensate; the atomic
environment's steep pressure gradient and oscillating vortex boundaries modify the
...
directly from the binding principle established in Paper~1~\cite{otto-overview}.
...
in the free medium.\ The electric field is the coherent spin-phase wave field
sourced by the bound core ring (Paper~4~\cite{otto-em}), and every atom generates a structured
near-field pressure environment through the nucleus's wave emission.\ A natural
question follows: if the Aether is a real medium that supports waves, and if atoms
create structured near-field environments in this medium, then atoms should function
as resonant cavities---and their energy levels should be discrete for the same reason
that a vibrating drumhead or an organ pipe supports only certain frequencies.

This section develops that connection.

\subsection{The Atomic Resonant Cavity}

Every nucleus generates a near-field coherent spin-phase wave field (its
electric field), whose time-averaged pressure envelope creates a velocity gradient
that is strongest near the nucleus and weakens with distance.\ This velocity gradient
constitutes a refractive environment: waves propagating tangentially are refracted
inward by the near-field pressure-gradient shear, analogous to how sound is bent by
wind gradients in the atmosphere.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Radial confinement.}\ The near-field pressure gradient creates an effective
    boundary.\ Near the nucleus, the strong gradient refracts the electron wave inward---confined.\
    At large distances, the near-field wave envelope weakens and the confining effect
    vanishes.\ Only certain radial profiles produce standing waves that remain bound
    (decaying at large~$r$) rather than radiating away into the medium.\ Non-resonant
    radial modes leak energy into the surrounding Aether and cannot persist.
\end{enumerate}
...
\emph{A note on the bracket constant:} classical thin-core results give
$-7/4$ (solid rotational core) and $-2$ (hollow core) as the ring
\emph{energy} constants, with $-1/2$ appearing as the hollow-core
\emph{velocity} constant; the value used here is retained for continuity
and should be verified against \cite{donnelly1991}.\ In the bound-core
model the core is filled and its energy is carried separately by
$f_{\mathrm{core}}$ (Paper~7(a)~\cite{otto-boundcore}), for which the
hollow-core energy convention ($-2$) is arguably the consistent choice;
the downstream effect of the bracket on the joint Coulomb consistency is
quantified there.
...
The same ring geometry yields two further results in
Paper~7(a)~\cite{otto-boundcore}: the Bohr magneton with $g = 2$, exactly,
from the azimuthal advance of the ring's topological winding (one winding
per Compton period), and Coulomb's constant from the same winding acting
as a spin-phase pump, $k\,e^2 = \rhoa\,\kappa_e^2\,r_{\mathrm{loop}}^2/8\pi$.\
The ring carries two clocks: the azimuthal winding advance at the
particle's Compton frequency $\omega_C = m_e c^2/\hbar$ (the magnetic
moment's clock), and a poloidal rotation of the winding at the medium's
frequency $\Omega = v(\xi)/\xi = m_A c^2/\hbar$, inherited from the vortex
flow at the healing boundary (the Coulomb pump's clock).
...
(Paper~5~\cite{otto-strong}) provides an independent set of constraints on
...
Paper~5~\cite{otto-strong} is the thermal disruption of these anchoring vortex
...
mechanism described in Paper~5~\cite{otto-strong}.
...
3.54\times10^{65}$~kg$^{-1}$m$^{-3}$ (e.g.\
$\rhoa \approx 5.0\times10^{11}$~kg/m$^3$ at $m_A = 666$~MeV/$c^2$); the
electron-mass convergence zone at that reference density is
$m_A \approx 616$--$644$~MeV/$c^2$.\ The compression ratio along the
Coulomb curve is $\approx 1.0\times10^{8}$ ($\log_{10}X \approx 8.0$),
about $50\%$ above the electron-mass-only value above; see Paper~7(a) §10
for the joint-consistency requirement on the core-energy fraction
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Fermi LAT and the spin-wave cutoff.}\ The density healing
    ...
    \emph{static} Coulomb interaction is insensitive to $q$: it uses only
    the kinetic spin-phase stiffness
    $K_\varphi = \rhoa\hbar^2/4m_A^2$; Paper~7(a)~\cite{otto-boundcore}.)\ The
...
\begin{itemize}[leftmargin=2em]
  \item Photons are spin-wave excitations of the spinor Aether condensate---transverse
    ...
    in free space.\
    Detection occurs when the wave enters the strong near-field pressure-gradient
    environment near matter, where DCE dynamics facilitate condensation into a
    localized quantum $E = h\nu$.\
  ...
  \item The static Coulomb interaction is carried by the same spin sector: each
    charged particle's bound core ring pumps the condensate's relative phase (one
    winding quantum per cycle), and the interaction of two such flux sources
    reproduces Coulomb's law with
    $k\,e^2 = \rhoa\,\kappa_e^2\,r_{\mathrm{loop}}^2/8\pi$
...
\begin{enumerate}[leftmargin=2em]
  \item Atoms function as resonant cavities in the Aether, with the nuclear electric field
    (the near-field coherent spin-phase wave envelope) creating a pressure gradient
    that confines electron waves through acoustic refraction.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Quantitative derivation of the near-field pressure profile near a nucleus.}\
    The resonant cavity model requires a specific pressure gradient around the
    nucleus to produce the correct energy levels.\ Can this profile be derived
    self-consistently from the theory's description of electric fields as coherent
    spin-phase wave fields (Paper~4~\cite{otto-em}), combined with the gravitational
    inflow from the nuclear mass?\ The profile must yield both the Bohr radius
    ($a_0 = 0.529$~\AA) and the $1/n^2$ energy scaling.
